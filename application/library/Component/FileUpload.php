...
class FileUpload
{
    ...
    {
        $this->ext_arr = array(
            'image' => array('gif', 'jpg', 'jpeg', 'png', 'bmp','swf','flv'),
            'flash' => array('swf', 'flv'),
            'media' => array('swf', 'flv', 'mp3', 'wav', 'wma', 'wmv', 'mid', 'avi', 'mpg', 'asf', 'rm', 'rmvb'),
            'file' => array('doc', 'docx', 'ppt','pptx'),
            'attachment' => array('xlsx','txt','docx','jpg','jpeg','png'),
        );
    ...
    private function _checkImageFileMime($tmp_name)
    {
        ...
        if(!in_array($fileInfo['mime'], $imageMimes)){
    ...
    public function run($type=1,$file='image')
    {
        //PHP上传失败
        ...
        if (empty($_FILES) === false) {
            ...
            //检查目录名
            $dir_name = empty($file) ? 'image' : trim($file);
            ...
            //检查扩展名
            if($type == 3) {
                if (in_array($file_ext, $this->ext_arr['attachment']) === false) {
